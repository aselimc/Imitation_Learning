...
def save_results(ep_rewards: List, results_dir="./results"):
    """
    ...
    results = dict()
    results["number_episodes"] = len(ep_rewards)
    results["episode_rewards"] = ep_rewards

    results["mean_all_episodes"] = np.array(ep_rewards).mean()
    results["std_all_episodes"] = np.array(ep_rewards).std()
